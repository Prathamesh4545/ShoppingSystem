...
const DealsPage = () => {
  ...
  return (
    <div className={`pt-16 px-4 md:px-10 lg:px-20 ${isDark ? 'bg-gray-900' : 'bg-gray-50'} min-h-screen`}>
      {/* Error Message */}
      {error && (
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {activeDeals.map((deal) => (
            <div
              key={deal.id}
              className={`p-6 border rounded-lg shadow-lg ${isDark ? 'bg-gray-800' : 'bg-white'} hover:shadow-xl transition-shadow duration-300`}
            >
              <LazyLoadImage
                src={
                  deal.imageUrl ||
