...
import { FaBars, FaHome, FaBox, FaPlusCircle, FaShoppingCart, FaUsers, FaChartLine } from "react-icons/fa";
...
const sidebarLinks = [
  ...
  { name: "Add Products", path: "/products/add", icon: <FaPlusCircle className="h-5 w-5" /> },
  { name: "Orders", path: "/orders", icon: <FaShoppingCart className="h-5 w-5" /> },
  { name: "Users", path: "/users", icon: <FaUsers className="h-5 w-5" /> },
  { name: "Analytics", path: "/analytics", icon: <FaChartLine className="h-5 w-5" /> },
];
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <Suspense fallback={<div className="text-center text-gray-500">Loading...</div>}>
        ...
        <div className={`transition-all ${sidebarOpen ? "ml-64" : "ml-20"} p-8`}>
          <UserStats />

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <ChartCard title="Sales Overview" ChartComponent={Bar} data={salesData} />
            <ChartCard title="Revenue Trends" ChartComponent={Line} data={revenueData} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <ChartCard title="Product Categories" ChartComponent={Doughnut} data={categoryData} />
          </div>
        </div>
      </Suspense>
    </div>
