...
const CartProvider = ({ children }) => {
  ...
  const fetchActiveDeals = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: "UPDATE_DEALS", payload: deals });
    } catch (error) {
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      // Apply deals to cart items
      const cartWithDeals = items.map(item => ({
        ...item,
        product: {
          ...item.product,
          dealInfo: deals.find(deal => 
            deal.products?.some(p => p.id === item.product.id)
          )
        }
      }));
  ...
  // Fetch the cart and deals when the component mounts or the user logs in
  useEffect(() => {
    if (isAuthenticated && hasRole("USER")) {
      fetchCart();
      // Set up periodic deal checks
      const dealCheckInterval = setInterval(fetchActiveDeals, 60000); // Check deals every minute
      return () => clearInterval(dealCheckInterval);
    }
  }, [isAuthenticated, fetchCart, fetchActiveDeals, hasRole]);

  // Calculate the total price of the cart with deals
