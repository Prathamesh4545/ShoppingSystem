...
public interface OrderRepo extends JpaRepository<Orders, Long> {
    ...
    @Query("SELECT NEW map(CAST(o.createdAt as date) as date, " +
           ...
           "COALESCE(SUM(CASE WHEN o.status != 'CANCELLED' THEN o.totalAmount ELSE 0 END), 0.0) as revenue) " +
           "FROM Orders o " +
    ...
    @Query("SELECT COALESCE(SUM(o.totalAmount), 0.0) FROM Orders o WHERE o.createdAt BETWEEN :start AND :end AND o.status != 'CANCELLED'")
    double findTotalAmountByOrderDateBetween(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("SELECT COALESCE(SUM(o.totalAmount), 0.0) FROM Orders o WHERE o.createdAt < :date AND o.status != 'CANCELLED'")
